...
async function createBasicTestData() {
  ...
  try {
    ...
    // Create test merchants first (no dependencies)
    ...
    // Create some basic profiles without loyalty numbers to avoid trigger issues
    ...
    for (let i = 0; i < testUsers.length; i++) {
      ...
      // Then create profile with explicit loyalty_number to avoid trigger issues
      await client.query(`
    ...
    console.log(`✅ Created ${testUsers.length} test profiles`);
