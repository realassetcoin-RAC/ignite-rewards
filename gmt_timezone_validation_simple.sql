-- GMT Timezone Validation for Referral Campaigns (Simplified Version)
-- This script adds server-side validation and automatic timezone conversion to GMT
-- WITHOUT dependencies on admin functions
...
-- 7. Create function to check if a campaign is currently active (in GMT)
...
-- 8. Add helpful comments to the referral_campaigns table
...
-- 9. Test the functions with sample data (optional - remove in production)
...
BEGIN
  -- Test with future dates to avoid validation errors
  test_start := (NOW() + INTERVAL '1 day')::TIMESTAMP WITH TIME ZONE; -- Tomorrow
  test_end := (NOW() + INTERVAL '30 days')::TIMESTAMP WITH TIME ZONE;  -- 30 days from now
