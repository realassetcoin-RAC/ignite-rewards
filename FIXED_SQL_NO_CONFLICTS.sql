-- =============================================================================
-- FIXED SUBSCRIPTION PLANS PERMISSION FIX (NO CONFLICT ERRORS)
-- =============================================================================
...
-- Step 12: Insert default subscription plans (using INSERT WHERE NOT EXISTS)
INSERT INTO api.merchant_subscription_plans (name, description, price_monthly, features, trial_days, is_active) 
SELECT 'Basic', 'Essential features for small businesses', 29.99, '["Basic analytics", "Customer loyalty tracking", "Email support"]'::jsonb, 30, true
WHERE NOT EXISTS (SELECT 1 FROM api.merchant_subscription_plans WHERE name = 'Basic');

INSERT INTO api.merchant_subscription_plans (name, description, price_monthly, features, trial_days, is_active) 
SELECT 'Premium', 'Advanced features for growing businesses', 79.99, '["Advanced analytics", "Custom branding", "Priority support", "API access"]'::jsonb, 30, true
WHERE NOT EXISTS (SELECT 1 FROM api.merchant_subscription_plans WHERE name = 'Premium');

INSERT INTO api.merchant_subscription_plans (name, description, price_monthly, features, trial_days, is_active) 
SELECT 'Enterprise', 'Full-featured solution for large businesses', 199.99, '["All Premium features", "Dedicated account manager", "Custom integrations", "24/7 phone support"]'::jsonb, 30, true
WHERE NOT EXISTS (SELECT 1 FROM api.merchant_subscription_plans WHERE name = 'Enterprise');

-- Step 13: Copy existing admin user from public.profiles to api.profiles if it exists (safe version)
DO $$
BEGIN
  -- Only insert if public.profiles exists and has admin users
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'profiles') THEN
    ...
    WHERE p.role = 'admin'
    AND NOT EXISTS (SELECT 1 FROM api.profiles WHERE id = p.id);
  END IF;
END $$;
