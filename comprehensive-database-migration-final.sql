-- ===========================================
-- COMPREHENSIVE DATABASE MIGRATION WITH DAO SYSTEM (FINAL CORRECTED)
-- ===========================================
-- This script safely adds missing tables and columns to existing database
-- Handles existing tables without conflicts and uses correct enum values
...
-- Create marketplace enums if they don't exist
DO $$ BEGIN
...
-- Add missing columns to marketplace_listings if they don't exist
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE public.marketplace_listings ADD COLUMN currency TEXT DEFAULT 'SOL';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   ...
                   AND table_schema = 'public') THEN
        ALTER TABLE public.marketplace_listings ADD COLUMN start_date TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   ...
                   AND table_schema = 'public') THEN
        ALTER TABLE public.marketplace_listings ADD COLUMN end_date TIMESTAMP WITH TIME ZONE;
    END IF;
END $$;
...
-- Insert sample marketplace listings (using correct enum values)
...
SELECT '✅ COMPREHENSIVE DATABASE MIGRATION COMPLETED!' as message;
...
SELECT 
    'Marketplace Listings Available' as info,
    COUNT(*) as count
