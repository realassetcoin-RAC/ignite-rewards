...
const DAODashboard = () => {
  ...
  const checkUserMembership = async () => {
    try {
      ...
      // Check if user is a member of any DAO
      const { data: membership } = await supabase
        .from('dao_members')
        .select(`
          *,
          dao_organizations!inner(*)
        `)
        .eq('user_id', user.id)
        .eq('is_active', true)
        .single();
        ...
        <Card className="relative z-10 w-full max-w-md card-gradient border-primary/20 backdrop-blur-md">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              You need to be a member of the RAC Rewards DAO to participate in governance.
            </p>
            <Button 
              onClick={() => navigate('/')}
