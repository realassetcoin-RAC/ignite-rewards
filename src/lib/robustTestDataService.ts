...
export class RobustTestDataService {
  /**
   * Create comprehensive test data with proper error handling
   */
  static async createComprehensiveTestData(): Promise<TestDataResult> {
    try {
      console.log('🚀 Creating comprehensive test data with robust error handling...');
      ...
      // Step 7: Update with real user IDs
   ...
   */
  private static async checkTablesExist(): Promise<TestDataResult> {
    ...
    if (missingTables.length > 0) {
      return {
        ...
        error: `The following tables do not exist: ${missingTables.join(', ')}. Please run the SQL script to create them.`
      };
    }
  ...
  private static async createDAOTestData(): Promise<any> {
    ...
    if (orgError) {
      throw new Error(`Failed to insert organization: ${orgError.message}`);
    }
  ...
  private static async createMerchantTestData(): Promise<any> {
    ...
    if (error) {
      throw new Error(`Failed to insert merchants: ${error.message}`);
    }
  ...
  private static async createMarketplaceTestData(): Promise<any> {
    ...
    if (error) {
      throw new Error(`Failed to insert marketplace listings: ${error.message}`);
    }
