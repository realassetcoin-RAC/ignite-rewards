...
BEGIN
    ...
    -- Update category constraint to include missing categories
...
CREATE POLICY "Users can view returns for their investments" ON public.investment_returns
    FOR SELECT USING (
        EXISTS (
            ...
            AND asset_investments.user_id = auth.uid()
        )
    );
...
-- 9. Update existing asset_initiatives with sample data
UPDATE public.asset_initiatives SET
    impact_score = CASE 
        WHEN category = 'environmental' THEN 9
        WHEN category = 'social' THEN 8
        WHEN category = 'technology' THEN 7
        ELSE 6
    END,
    risk_level = CASE 
        WHEN category = 'environmental' THEN 'low'
        WHEN category = 'social' THEN 'medium'
        WHEN category = 'technology' THEN 'medium'
        ELSE 'high'
    END,
    expected_return = CASE 
        WHEN category = 'environmental' THEN 8.5
        WHEN category = 'social' THEN 7.0
        WHEN category = 'technology' THEN 9.0
        ELSE 6.5
    END,
    min_investment = 100.00,
    max_investment = 100000.00,
    current_funding = 0.00,
    target_funding = 1000000.00,
