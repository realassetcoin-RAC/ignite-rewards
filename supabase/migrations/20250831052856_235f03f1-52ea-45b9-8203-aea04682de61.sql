-- Update the admin access functions with correct enum schema references
...
AS $$
  SELECT EXISTS (
    ...
    WHERE id = auth.uid() AND role = 'admin'::api.app_role
  );
$$;
...
CREATE OR REPLACE FUNCTION public.has_role(_user_id uuid, _role api.app_role)
RETURNS boolean
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = 'api, public'
