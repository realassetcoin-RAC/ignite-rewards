-- HOTFIX: Profiles, RPC wrappers in api schema, Referrals schema + RLS
-- Safe to run multiple times (guards for duplicates where possible)

-- 0) Ensure api schema exists
CREATE SCHEMA IF NOT EXISTS api;

-- 1) Minimal api.profiles view mapping to public.profiles
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relkind IN ('v','r') AND c.relname = 'profiles' AND n.nspname = 'api'
  ) THEN
    EXECUTE $v$
...
-- 2) api.check_admin_access wrapper (PostgREST searches api.* by default in some configs)
...
BEGIN
  ...
  BEGIN
    ...
    WHERE id = user_id
    LIMIT 1;
    IF FOUND AND user_role = 'admin' THEN RETURN true; END IF;
  EXCEPTION WHEN OTHERS THEN NULL; END;
...
-- 3) api.get_current_user_profile wrapper
...
BEGIN
  ...
  BEGIN
    RETURN QUERY
    SELECT
      p.id, p.email, p.full_name, p.role::text,
...
-- 4) Ensure profiles exist (admin + backfill)
DO $$
DECLARE
  u RECORD;
  admin_user_id uuid;
...
-- 6) Optional: admin policy for virtual_cards
DO $$ BEGIN
