-- Update all functions to work with api schema using explicit type references
CREATE OR REPLACE FUNCTION api.has_role(_user_id uuid, _role public.app_role)
RETURNS boolean
LANGUAGE sql
...
AS $function$
  SELECT EXISTS (
    SELECT 1
    FROM api.profiles
    WHERE id = _user_id AND role = _role::text::public.app_role
  )
$function$;
...
AS $function$
  SELECT EXISTS (
    ...
    WHERE id = COALESCE(user_id, auth.uid()) AND role = 'admin'::public.app_role
  );
$function$;
