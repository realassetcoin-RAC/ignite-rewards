...
-- RLS Policies for contact_tickets
...
CREATE POLICY "Admins can view all interactions" ON chatbot_interactions
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can manage tags" ON issue_tags
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
-- RLS Policies for slack_integration_settings (admin only)
CREATE POLICY "Admins can manage Slack settings" ON slack_integration_settings
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
