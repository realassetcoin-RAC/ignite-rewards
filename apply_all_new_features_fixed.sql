-- Apply all new features to the database (FIXED VERSION)
...
-- Insert sample data for testing (only if tables exist and have data)
DO $$
DECLARE
    v_merchants_primary_key text;
    v_merchants_has_user_id boolean := false;
    ...
    v_asset_initiative_count integer := 0;
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'merchants') THEN
        ...
        -- Check if merchants table has any data
        EXECUTE format('SELECT COUNT(*) FROM public.merchants') INTO v_merchant_count;
        
        IF v_merchants_primary_key IS NOT NULL AND v_merchant_count > 0 THEN
            IF NOT EXISTS (SELECT 1 FROM public.merchant_custom_nfts LIMIT 1) THEN
                EXECUTE format('INSERT INTO public.merchant_custom_nfts (
                    merchant_id,
                    nft_name,
                    description,
                    image_url,
                    ...
                    (SELECT %I FROM public.merchants LIMIT 1),
                    ''VIP Member Card'',
                    ...
                    now() + interval ''1 year'',
                    true
                )', v_merchants_primary_key);
                RAISE NOTICE 'Inserted sample merchant custom NFT';
            ELSE
                RAISE NOTICE 'Sample merchant custom NFT already exists - skipping';
            END IF;
        ELSE
            IF v_merchant_count = 0 THEN
                RAISE NOTICE 'Skipping sample merchant custom NFT - merchants table is empty';
            ELSE
                RAISE NOTICE 'Skipping sample merchant custom NFT - could not determine primary key';
            END IF;
        END IF;
    ELSE
    ...
    -- Check if asset_initiatives table exists and has data
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'asset_initiatives') THEN
        -- Check if asset_initiatives table has any data
        SELECT COUNT(*) INTO v_asset_initiative_count FROM public.asset_initiatives;
        
        IF v_asset_initiative_count > 0 THEN
            ...
            IF v_user_count > 0 THEN
                ...
                ELSE
                    RAISE NOTICE 'Sample asset wallet already exists - skipping';
                END IF;
            ELSE
                RAISE NOTICE 'Skipping sample asset wallet - no users found in auth.users';
            END IF;
        ELSE
            RAISE NOTICE 'Skipping sample asset wallet - asset_initiatives table is empty';
        END IF;
    ELSE
