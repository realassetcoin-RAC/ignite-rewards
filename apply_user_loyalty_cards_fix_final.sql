-- FINAL VERSION: Apply this SQL directly in your Supabase dashboard SQL editor
-- This version handles all function signature conflicts and existing objects properly
...
-- 5. Drop ALL existing generate_loyalty_number functions to avoid conflicts
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Drop all variations of generate_loyalty_number functions
    FOR func_record IN 
        SELECT n.nspname as schemaname, p.proname as functionname, pg_get_function_identity_arguments(p.oid) as args
        FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
        WHERE p.proname = 'generate_loyalty_number'
    ...
    LOOP
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'Could not drop function %.%(%s): %', 
                            func_record.schemaname, 
                            func_record.functionname, 
                            func_record.args, 
                            SQLERRM;
